...
class ReportUtilsMixin(ReportBuilderFieldUtils, FilterQueryMixin):
    ...
    @staticmethod
    def _set_multiple_title(database_values, value_prefix, fields, text, null_text=''):
        """The title of one column generated by a "multiple columns" split.

        Lives here rather than on the table and chart mixins separately: they had an identical
        copy each, which is the shape a change like this goes wrong in -- fix the table's copy and
        a bar chart split on the same nullable field keeps heading its NULL series "None" while
        the breakdown table beside it uses the configured name.

        `null_text` is keyed on the SPLIT VALUE ITSELF, not on the title fields being empty. Those
        are two different questions: a real related row whose name happens to be NULL is not the
        absence of a related row, and a builder that declares no title fields at all would
        otherwise have every one of its columns declared null.
        """
        if null_text and database_values.get(value_prefix) is None:
            return null_text
        results = {}
        for field in fields:
